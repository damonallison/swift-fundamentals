...
/// Xcode recognizes 3 special comment markers in Swift : MARK: TODO: FIXME:
///
/// Each comment marker adds a delimiter and item to Xcode's "jump bar"
/// file outliner at the top of the editor window.
///
/// Adding a single `-` after the comment marker tells Xcode to also add a
/// horizontal line above the item as well, delmiiting it within within the
/// jump bar.
///
/// `// MARK:` adds a "marker" item in the jump bar.
...
class CommentTests : XCTestCase {
    ...
    /// - returns: Nothing!
    func testBasics(_ name: String, iq: Int) {
        print("\(name) and \(iq)")
    }
